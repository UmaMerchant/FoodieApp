...
function RestaurantForm({ onClose, editData }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editData) {
        await axios.put('http://localhost:9095/api/restaurants/update', formDataToSend, { headers });
        setSuccess('Restaurant updated successfully!');
      } else {
        await axios.post('http://localhost:9095/api/restaurants/add', formDataToSend, { headers });
